...
detection_with_class* Yolo::get_actual_detections(detection *dets, int dets_num, float thresh, int* selected_detections_num)
{
    int selected_num = 0;
...
void Yolo::draw_detections_v3(ImageUtil::ImageData im, detection *dets, int num, float thresh, char** names,
...
void Yolo::yolov2_fuse_conv_batchnorm(network* net)
{
    int j;
...
void Yolo::calculate_binary_weights(network* net)
{
    int j;
    for (j = 0; j < net->n; ++j)
    {
...
bool Yolo::test(char *cfgfile, char *weightfile, char *filename, float thresh)
{
    if (!filename)
        return false;
    ...
    //for (j = 0; j < l.side*l.side*l.n; ++j) probs[j] = calloc(l.classes, sizeof(float *));
        ...
        draw_detections_v3(im, dets, nboxes, thresh, voc_names, alphabet, pYolo->getClasses(), ext_output);
...
bool Yolo::train(const char* datacfg, char *cfgfile, char *weightfile)
{
...
void Yolo::fill_network_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, detection *dets, int letter)
{
    int j;
    for (j = 0; j < net->n; ++j)
    {
...
detection * Yolo::get_network_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, int *num, int letter)
{
...
bool Yolo::readWeightFile(network *net, char *filename, int cutoff)
{
...
JJ::network* Yolo::readConfigFile(const char* filename, int batch)
{
    IniParser parser;
...
JJ::LAYER_TYPE Yolo::string_to_layer_type(const std::string& type)
{
...
bool Yolo::parseNetOptions(const IniParser* pIniParser, JJ::network* net)
{
...
// detect on CPU
void Yolo::network_predict_cpu(network* net, float *input)
{
     float* pInput = input;
